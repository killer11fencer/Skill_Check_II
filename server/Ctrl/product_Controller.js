module.exports = {
    create(req, res) {
        ...
        db.create_product([name, price, image_url]).then(result => res.status(200).send(result))
    ...
    update(req,res) {
        ...
        const {id} = req.params
        const {name,price,image_url} = req.body
        db.update_product([id,name,price,image_url]).then(result => res.status(200).send(result))
